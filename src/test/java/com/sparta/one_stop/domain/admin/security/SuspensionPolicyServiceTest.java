...
class SuspensionPolicyServiceTest {
 @Test void 만료된_정지는_접근시_자동해제된다(){
  var repo=mock(UserSecurityActionRepository.class);var audit=mock(SecurityAuditService.class);var users=mock(UserRepository.class);var user=mock(User.class);var action=mock(UserSecurityAction.class);
  ...
  new SuspensionPolicyService(repo,audit,users).validateOrRelease(user);
