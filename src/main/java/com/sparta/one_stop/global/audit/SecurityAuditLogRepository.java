...
public interface SecurityAuditLogRepository extends JpaRepository<SecurityAuditLog,Long> {
    ...
    Page<SecurityAuditLog> findBySeverityAndOccurredAtAfterOrderByOccurredAtDesc(
        SecuritySeverity severity, LocalDateTime since, Pageable pageable);
    Page<SecurityAuditLog> findBySeverityInAndOccurredAtAfterOrderByOccurredAtDesc(
        List<SecuritySeverity> severities, LocalDateTime since, Pageable pageable);

    @Query("SELECT l.category, COUNT(l) FROM SecurityAuditLog l WHERE l.occurredAt > :since GROUP BY l.category")
