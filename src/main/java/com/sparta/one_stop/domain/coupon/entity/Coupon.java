...
public class Coupon extends BaseEntity {
    ...
    public void validateIssuable(LocalDateTime now) {
        if (this.status != CouponStatus.ACTIVE) {
            throw new CustomException(ErrorCode.COUPON_033);
        }
    ...
    private void validateOrderPrice(Long orderPrice) {
        if (orderPrice == null || orderPrice < 0) {
            throw new CustomException(ErrorCode.COUPON_031);
        }
    }
    ...
    private void validate(
        ...
        if (name == null || name.isBlank()) {
            throw new CustomException(ErrorCode.COUPON_020);
        }

        if (discountType == null) {
            throw new CustomException(ErrorCode.COUPON_021);
        }

        if (discountValue == null || discountValue <= 0) {
            throw new CustomException(ErrorCode.COUPON_022);
        }

        if (minOrderPrice == null || minOrderPrice < 0) {
            throw new CustomException(ErrorCode.COUPON_023);
        }

        if (totalQuantity == null || totalQuantity <= 0) {
            throw new CustomException(ErrorCode.COUPON_024);
        }

        if (startAt == null) {
            throw new CustomException(ErrorCode.COUPON_025);
        }

        if (expiredAt == null) {
            throw new CustomException(ErrorCode.COUPON_026);
        }

        if (!expiredAt.isAfter(startAt)) {
            throw new CustomException(ErrorCode.COUPON_027);
        }
    ...
    private void validateDiscountPolicy(
        ...
        if (discountType == CouponDiscountType.RATE) {
            if (discountValue > 100) {
                throw new CustomException(ErrorCode.COUPON_028);
            }

            if (maxDiscountPrice == null || maxDiscountPrice <= 0) {
                throw new CustomException(ErrorCode.COUPON_029);
            }
        }

        if (discountType == CouponDiscountType.FIXED) {
            if (maxDiscountPrice != null) {
                throw new CustomException(ErrorCode.COUPON_030);
            }
        }
    }
    ...
    public Long calculateDiscountPrice(Long orderPrice) {
        ...
        if (orderPrice < this.minOrderPrice) {
            throw new CustomException(ErrorCode.COUPON_005);
        }
